...
const MegaMenu = ({ data }) => {
    ...
    return (
        ...
        <div>
                ...
                {data && (
                    <ul style={{ listStyleType: 'none', padding: '0' }}>
                        {data.map((subitem, subindex) => (
                            <li
                                key={`${subindex}si`}
                                onMouseEnter={() => subHandleHover(`${subindex}si`, subitem, subindex)}
                                className='hover:bg-blue-gray-100 transition duration-300 flex justify-between items-center'
                            >
                                <a href={subitem.link} className='text-medium p-4 '>{subitem.brand}</a>
            ...
            {activeSubcategories === `${subIndex}si` && subItem?.AllPerfume && subItem?.AllPerfume.length > 0 && (
                <div
                    style={{
                        position: 'absolute',
                        top: '0',
                    ...
                    <ul style={{ listStyleType: 'none', padding: '0' }}>
                        {subItem.AllPerfume.map((subitem2, subindex2) => (
                            <li key={subindex2} className=' hover:bg-blue-gray-100 transition duration-300 flex justify-between'>
                                <a href={subitem2._id} className='text-medium p-4'>{subitem2.perfume}</a>
                            </li>
                        ))}
                    </ul>
                </div>
