...
const RatingResult = ({ productId }) => {
    ...
    const results = [
        {
            name: "Very Good",
            icons: <svg className='h-8 stroke-[5px]' viewBox="0 0 128 128" fill="currentColor"><path d="M109.2,18.8C84-6.3,43.6-6.3,18.8,18.8c-25.1,25.125.1,65.6,0,90.3c25.1,25.1,65.6,25.1,90.3,0
    ...
    return (
        ...
        // latest code
        <div
            className="grid md:grid-cols-2 gap-6 sm:gap-10 md:gap-16 w-full mx-auto md:mx-0 md:w-full px-4 sm:px-8 lg:px-12">
            {ratingData?.map((item, ind) => (
                <div key={ind} className="flex flex-col gap-4">
                    {/* Header Section */}
                    <div
                        className="font-semibold grid place-items-center text-center text-sm sm:text-lg py-2 text-[#2071B2] capitalize">
                    ...
                    <div className="flex justify-center gap-3 sm:gap-4 h-full">
                        {item.status.map((stats, idx) => (
                            ...
                            >
                                ...
                                <div
                                    className={`text-xs sm:text-[12px] font-bold text-wrap text-center ${
                                        stats.num >= 1 ? "text-pink-400" : "text-gray-400"
                                    }`}
                                >
                                    {stats?.name}
                                </div>
                    ...
                    <div className="w-full flex justify-center ">
                        <input
                            ...
                            step={1}
                            value={
                                item.status.findIndex(
                                    (sta) => sta.num === Math.max(...item.status.map((s) => s.num))
                                ) + 1
                            }
                            type="range"
                            onChange={(e) => {
                                const selectedIndex = e.target.value - 1;
                                const selectedStatus = item.status[selectedIndex];
                                updateRating(item, selectedStatus);

                                // Dynamically update the progress bar for the selected status
                                selectedStatus.num = (selectedIndex + 1) * 20; // Example: Update sta.num based on range value
...
<div className="space-y-3">
  {(() => {
    // Find the maximum `sta.num` value in the array
    const maxStaNum = Math.max(...(item?.status.map((sta) => sta.num) || []));

    return item?.status.map((sta, idx) => (
      <div key={idx} className="grid grid-cols-[6rem_auto]">
        <span
          className={`capitalize text-wrap w-[6rem] text-xs sm:text-sm ${
            sta.num === maxStaNum ? "text-pink-400" : "text-gray-400"
          }`}
        >
          {sta.name}
        </span>
        <div className="w-full flex items-center gap-3">
          ...
          <div className="bg-slate-300 relative w-[80%] md:w-full rounded-3xl h-[6px] sm:h-[8px]">
            <div
              style={{ width: `${sta.num}%` }}
              className={`absolute rounded-3xl h-[6px] sm:h-[8px] ${
                sta.num === maxStaNum ? "bg-pink-300" : "bg-gray-300"
              }`}
            ></div>
          </div>
        </div>
...
//   async function getTotalRatingsAnalyst(perfumeId) {
//     const result = await axios.get(
//       `${process.env.NEXT_PUBLIC_API_URL}/api/v1/review-analytics/${perfumeId}`
//     );
//     console.log("getTotalRatingsAnalyst", result.data.data);
//    setPerfumeAnalytics(result?.data?.data);}
// useEffect(() => {
//     if (id) {
//       getTotalRatings(id);
...
//   const getReviewByUserId = async () => {
//     const result = await axios.get(
//       `${process.env.NEXT_PUBLIC_API_URL}/api/v1/review/user/${user?._id}`
//     );
//     // console.log(result, "Reslt")
//     setResult(result?.data?.data[0]);
//   };
...
//       `${process.env.NEXT_PUBLIC_API_URL}/api/v1/productReviewCount/${id}`,
//       { ...payload }
//     );
//     getReviewByUserId();
//     getTotalRatings(id);
...
//           ></path>{" "}
//           <path
//             d="M90,72.5c-1.7-0.9-3.8,0-4.7,1.7c-3.4,8.1-11.8,13.2-20.7,13.2c-0.3,0-0.6,0-0.6,0
//             c-9,0-17.6-5.1-21-13.2c-0.9-1.7-3.1-2.6-4.8-1.7c-1.7,0.9-2.4,2.8-1.6,4.9C41.3,87.9,52,94.7,64,94.7c0,0,0.1,0,0.2,0
//             c12.4,0,22.9-6.6,27.6-17.3C92.6,75.3,91.7,73.4,90,72.5z"
//           ></path>{" "}
//           <path
//             d="M44,60.5l2.1,2.1l2.1-2.1c8.1-7.2,13.2-11.9,13.2-17.5c0-4.7-3.8-8.5-8.5-8.5c-2.6,0-5.1,1.3-6.8,3.4
//             c-1.7-2.1-4.3-3.4-6.8-3.4c-4.7,0-8.5,3.8-8.5,8.5C30.8,49,35.9,53.7,44,60.5z"
//           ></path>{" "}
//           <path
//             d="M88.7,34.5c-2.6,0-5.1,1.3-6.8,3.4c-1.7-2.1-4.3-3.4-6.8-3.4c-4.7,0-8.5,3.8-8.5,8.5
//             c0,6,5.1,10.6,13.2,17.5l2.1,2.1l2.1-2.1c8.1-7.2,13.2-11.9,13.2-17.5C97.2,38.4,93.4,34.5,88.7,34.5z"
//           ></path>
//         </svg>
//       ),
//     },
...
//           <circle cx="81.5" cy="47" r="7.7"></circle>{" "}
//           <path
//             d="M36.7,85.7c0,2.3,1.9,4.2,4.2,4.2h46.2c2.3,0,4.2-1.9,4.2-4.2c0-2.3-1.9-4.2-4.2-4.2H40.9
//             C38.6,81.5,36.7,83.4,36.7,85.7L36.7,85.7z"
//           ></path>
//         </svg>
//       ),
//     },
...
//           ></path>{" "}
//           <path
//             d="M64.2,72.7c-0.1,0-0.2,0-0.2,0c-12,0-22.7,6.5-27.3,17c-0.9,2.1-0.1,3.8,1.6,4.7c1.7,0.9,3.9,0,4.8-1.7
//             c3.4-8.1,12-13.2,21-13.2c0,0,0.3,0,0.6,0c8.9,0,17.3,5.1,20.7,13.2c0.9,1.7,3,2.6,4.7,1.7c1.7-0.9,2.6-2.5,1.8-4.7
//             C87.1,79,76.5,72.7,64.2,72.7z"
//           ></path>{" "}
//           <path
//             d="M92.2,32.1c-0.2,0-4.3,1.1-8.5,3.2c-4.4,2.2-9.6,6-9.9,11.7c0,0.1,0,0.3,0,0.4c0,4.3,3.5,7.7,7.7,7.7
//             c4.3,0,7.7-3.5,7.7-7.7c0-2-0.8-3.9-2.1-5.3c2.2-1.1,4.8-2,6.9-2.6c2.1-0.5,3.3-2.6,2.8-4.7C96.4,32.8,94.3,31.5,92.2,32.1z"
//           ></path>{" "}
//           <path
//             d="M38.7,47.4c0,4.3,3.5,7.7,7.7,7.7c4.3,0,7.7-3.5,7.7-7.7c0-0.1,0-0.3,0-0.4c-0.2-5.7-5.5-9.5-9.9-11.7
//             c-4.2-2.1-8.3-3.1-8.5-3.2c-2.1-0.5-4.2,0.7-4.7,2.8c-0.5,2.1,0.7,4.2,2.8,4.7c2.1,0.5,4.6,1.5,6.9,2.6
//             C39.5,43.5,38.7,45.4,38.7,47.4z"
//           ></path>
//         </svg>
//       ),
//     },
...
//           name: "Very Weak ",
//           isUserSelected: user
//             ? modifyStr(result?.longevity || "null") === "veryweak"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.longevity.k) === "veryweak",
//           num: perfumeRatings?.longevity?.veryWeak,
...
//           name: "Weak",
//           isUserSelected: user
//             ? modifyStr(result?.longevity || "null") === "weak"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.longevity.k) === "weak",
//           num: perfumeRatings?.longevity?.weak,
...
//           name: "Moderate",
//           isUserSelected: user
//             ? modifyStr(result?.longevity || "null") === "moderate"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.longevity.k) === "moderate",
//           num: perfumeRatings?.longevity?.moderate,
...
//           name: "Long Lasting",
//           isUserSelected: user
//             ? modifyStr(result?.longevity || "null") === "longlasting"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.longevity.k) === "longlasting",
//           num: perfumeRatings?.longevity?.longLasting,
...
//           name: "Eternal",
//           isUserSelected: user
//             ? modifyStr(result?.longevity || "null") === "eternal"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.longevity.k) === "eternal",
//           num: perfumeRatings?.longevity?.eternal,
...
//           name: "Intimate",
//           isUserSelected: user
//             ? modifyStr(result?.sillage || "null") === "intimate"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.sillage.k) === "intimate",
//           num: perfumeRatings?.sillage?.intimate,
...
//           name: "No Vote",
//           isUserSelected: user
//             ? modifyStr(result?.sillage || "null") === "novote"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.sillage.k) === "novote",
//           num: perfumeRatings?.sillage?.noVote,
...
//           name: "Moderate",
//           isUserSelected: user
//             ? modifyStr(result?.sillage || "null") === "moderate"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.sillage.k) === "moderate",
//           num: perfumeRatings?.sillage?.moderate,
...
//           name: "Strong",
//           isUserSelected: user
//             ? modifyStr(result?.sillage || "null") === "strong"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.sillage.k) === "strong",
//           num: perfumeRatings?.sillage?.strong,
...
//           name: "Enormous",
//           isUserSelected: user
//             ? modifyStr(result?.sillage || "null") === "enormous"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.sillage.k) === "enormous",
//           num: perfumeRatings?.sillage?.enormous,
...
//           name: "Way Over Priced",
//           isUserSelected: user
//             ? modifyStr(result?.priceValue || "null") === "wayoverpriced"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.priceValue.k) === "wayoverpriced",
//           num: perfumeRatings?.priceValue?.wayOverPriced,
...
//           name: "Over Priced",
//           isUserSelected: user
//             ? modifyStr(result?.priceValue || "null") === "overpriced"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.priceValue.k) === "overpriced",
//           num: perfumeRatings?.priceValue?.overPriced,
...
//           name: "Ok",
//           isUserSelected: user
//             ? modifyStr(result?.priceValue || "null") === "ok"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.priceValue.k) === "ok",
//           num: perfumeRatings?.priceValue?.ok,
...
//           name: "Good Value",
//           isUserSelected: user
//             ? modifyStr(result?.priceValue || "null") === "goodvalue"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.priceValue.k) === "goodvalue",
//           num: perfumeRatings?.priceValue?.goodValue,
...
//           name: "Great Value",
//           isUserSelected: user
//             ? modifyStr(result?.priceValue || "null") === "greatvalue"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.priceValue.k) === "greatvalue",
//           num: perfumeRatings?.priceValue?.greatValue,
...
//           // isUserSelected: modifyStr(result?.gender) === 'female',
//           isUserSelected: user
//             ? modifyStr(result?.gender || "null") === "female"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.gender.k) === "female",
//           num: perfumeRatings?.gender?.female,
...
//           name: "More Female",
//           isUserSelected: user
//             ? modifyStr(result?.gender || "null") === "morefemale"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.gender.k) === "morefemale",
...
//           name: "Unisex",
//           isUserSelected: user
//             ? modifyStr(result?.gender || "null") === "unisex"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.gender.k) === "unisex",
...
//           name: "More Male",
//           isUserSelected: user
//             ? modifyStr(result?.gender || "null") === "moremale"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.gender.k) === "moremale",
...
//           name: "Male",
//           isUserSelected: user
//             ? modifyStr(result?.gender || "null") === "male"
//               ? true
//               : false
//             : modifyStr(perfumeAnalytics?.gender.k) === "male",
...
//   // Calculate the maximum rating for this status item based on 'sta.num'
//   const maxRating = Math.max(...item.status.map(status => status.num));
...
//           className="w-full h-[12px] sm:h-[14px] md:h-[16px] rounded-md cursor-pointer"
//           min={1}
//           max={5} // Use the calculated maxRating for the range input
//           step={1}
//           value={maxRating} // Bind value to sta.num (the current rating value)
//           type="range"
//           onChange={(e) => {
//             const newRating = e.target.value; // Get the new rating from the input
//             console.log("Rating for", item.name, "is:", newRating);
            
//             // Update the rating for the item
//             updateRating(item, item.status[newRating - 1]); 
//             setEmoji(newRating); // Update emoji state (if needed)
//           }}
//         />
//       </div>
//     </div>
