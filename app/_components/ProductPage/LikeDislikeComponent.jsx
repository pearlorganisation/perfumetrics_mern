...
const LikeDislikeComponent = React.memo(({ historyMap, productId, data }) => {
  ...
  const getLikeDisLike = async () => {
    const result = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/api/v1/perfume/like-dislike/${productId}`
    );
    ...
    <div className="flex justify-center   mt-6  ">
      ...
      >
        <div className="relative">
          {isLoading ? (
            <div className="absolute w-full h-full border-t-2 animate-spin rounded-full border-pink-500"></div>
          ) : (
            ""
          )}
          <IoHeart
            className={`${
              userHistory?.get(productId)?.vote === 1
                ...
                : ""
            }  border-2 size-12 border-black rounded-full p-1 text-pink-300`}
            size={38}
          />
        </div>
        ...
        <div className="relative">
          ...
          <Image
            className={`${
              ...
              isLoading ? "ring-4 ring-pink-500" : ""
            } border-2 size-12 border-black rounded-full`}
            src="/likes.svg"
            width={50}
            height={50}
            alt=""
