        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col items-center p-6 space-y-6 bg-gray-100 min-h-screen">
            ...
            <div className="w-full max-w-lg rounded-lg">
                ...
                <input
                    ...
                    className={`w-full px-3 py-2 border ${errors.perfumeName ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm focus:outline-none focus:ring focus:ring-indigo-200`}
                />
            ...
            <div className="w-full max-w-lg rounded-lg">
                <div className="flex items-center justify-center w-full">
                    ...
                    >
                        ...
                        <input type="file" id="photo-upload" accept="image/*" multiple onChange={handleImageUpload} className="hidden" />
                    </label>
                </div>
            </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 border border-gray-300 w-full p-2">
                        {selectedImages.map((image, index) => (
                            <div key={index} className="relative shadow-lg">
                                <img src={image} alt={`Preview ${index + 1}`} className="w-full h-auto rounded-md shadow-sm" />
                                <button
                                    type="button"
                                    onClick={() => handleRemoveImage(index)}
                                    className="absolute top-1 right-1 bg-red-500 text-white rounded-full hover:bg-red-700"
                                >
                                    &times;
                                </button>
                            </div>
